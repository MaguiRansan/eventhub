...
class EventsListViewTest(BaseEventTestCase):
    ...
    def test_events_view_filtering(self):
        ...
        # Test filtering by category - should only return event1 (which has category1)
        ...
        # Test filtering by venue - should return both events (event1 and event2)
        response = self.client.get(reverse("events") + "?venue=" + str(self.venue.pk))
        self.assertEqual(len(response.context["events"]), 2)
        
        # Create past event for testing past events filter
        ...
        # Test filtering past events - should only return the past event
        # First, let's check how many total events we have
        total_events = Event.objects.count()
        self.assertEqual(total_events, 3)  # event1, event2, and past_event
        
        # Now test the past events filter - this should return only past events
        response = self.client.get(reverse("events") + "?mostrar_pasados=true")
        events_returned = len(response.context["events"])
        
        # If the filter is working correctly, it should return only past events (1)
        # If it's not working, it might return all events (3) or future events (2)
        if events_returned == 3:
            # The filter might not be working properly, returning all events
